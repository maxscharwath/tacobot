...
content = content.replace(/const orderCard = function\(orderData\) \{[\s\S]*?orderItemsHtml = function/g, 
  'const orderCard = function(orderData) {\n          const orderItemsHtml = function');

// Also fix the duplicate orderCard assignment
content = content.replace(/orderItemsHtml = function\(order\) \{[\s\S]*?orderCard = document\.createElement/g,
...
console.log('Final variable replacements complete!');
