...
content = content.replace(/^(\s+)(orderItemsHtml|orderCard)\s*=\s*function/gm, '$1const $2 = function');
...
content = content.replace(/orderCard\s*=\s*function\(orderData\)\s*\{[\s\S]*?orderItemsHtml\s*=\s*function/g, 
  'orderCard = function(orderData) {\n          const orderItemsHtml = function');

// Step 6: Clean up multiple assignments on same line
content = content.replace(/orderCard\s*=\s*function\(orderData\)\s*\{\s*orderItemsHtml\s*=\s*function/g, 
  'orderCard = function(orderData) {\n          const orderItemsHtml = function');
...
content = content.replace(/(const orderCard = function\(orderData\) \{)\s*orderItemsHtml = function/g, 
  '$1\n          const orderItemsHtml = function');
